 ...
 */
class DirectionPolylinePlugin(
    val mapmyIndiaMap: MapplsMap,
    mapView: MapView,
    private var directionsCriteria: String?
    ...
    fun createPolyline(latLngs: List<LatLng>) {
        ...
        if (directionsCriteria!!.equals(DirectionsCriteria.PROFILE_WALKING, ignoreCase = true)) {
            lineLayer!!.setProperties(
                lineDasharray(arrayOf(widthDash, gapDash)),
                lineColor(Color.BLACK)
            )
        } else {
            lineLayer!!.setProperties(
                lineDasharray(arrayOf()),
        ...
        mapmyIndiaMap.getStyle {
            if (it.getSource(UPPER_SOURCE_ID) == null) {
                polylineSource = GeoJsonSource(
                    UPPER_SOURCE_ID, featureCollection,
